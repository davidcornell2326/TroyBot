...
public class TroyBot extends ListenerAdapter {
    ...
    private void mentionsTroyBot(MessageChannel channel, Message msg) {
        ...
        boolean flag = false;
        if (s.contains("hi")) {
            channel.sendMessage(mention + " Hi!").queue();
            flag = true;
        }
        if (s.contains("hello")) {
            channel.sendMessage(mention + " Hello!").queue();
            flag = true;
        }
        if (s.contains("help")) {
            help(channel, s);
        ...
        if (s.contains("thanks") || s.contains("thank you") || s.contains("thank u")) {
            channel.sendMessage(mention + " You're welcome!").queue();
            flag = true;
        }
        if (s.contains("stop")) {
            channel.sendMessage(mention + " Nothing can stop me! Except maybe a kill command on my virtual machine").queue();
            flag = true;
        }
        if (s.contains("love you") || s.contains("love u")) {
    ...
    public void startCasesChecks() {
        try {
            URL url = new URL("https://api.aditya.diwakar.io/gt-jpj/cases");
    ...
    public void cases(MessageChannel channel) {
        try {
            URL url = new URL("https://api.aditya.diwakar.io/gt-jpj/cases");
            // read text returned by server
            BufferedReader in = new BufferedReader(new InputStreamReader(url.openStream()));
            ...
            String msgToSend = "Covid Cases Manual Update:";
            String total = null;
            String reported = null;
            String date = null;
            for (int i = length - 20; i > 0; i--) {
                if (line.charAt(i) == '}' && total == null) { // } occurs after total #
                    int iCopy = i;
                    ...
                    total = line.substring(i + 1, iCopy);
                } else if (line.charAt(i) == ',' && reported == null) {
                    int iCopy = i;
                    boolean numeric = true;
                    while (numeric) {
                        i--;
                    ...
                    reported = line.substring(i + 1, iCopy);
                } else if (line.charAt(i) == ',' && date == null) {
                    for (int j = i - 2; j > 0; j--) {
                        if (line.charAt(j) == '\"') {
                            date = line.substring(j + 1, i - 1);
                            break;
                        }
                    }
                }
    ...
    public void casesScheduled(MessageChannel channel) {
        try {
            URL url = new URL("https://api.aditya.diwakar.io/gt-jpj/cases");
            // read text returned by server
            BufferedReader in = new BufferedReader(new InputStreamReader(url.openStream()));
            ...
            if (!line.equals(currentCases)) {
                currentCases = line;
                int length = line.length();
                String msgToSend = "@everyone\nCovid Cases Daily Update:";
                String total = null;
                String reported = null;
                String date = null;
                for (int i = length - 20; i > 0; i--) {
    ...
    public void whatsthegoodword(MessageChannel channel) {
        channel.sendMessage("To H3\uD83C\uDfD2\uD83C\uDfD2 With georgia!").queue();
    }

    public void bee(MessageChannel channel) {
...
//            int start = i * 2000;
//            int stop = start + 2000;
//            pieces[i] = s.substring(start, stop);
//        }
//        pieces[25] = s.substring(50000, s.length());
//        for (String piece : pieces) {
//            channel.sendMessage(piece).queue();
//        }
    }

    public void plagueis(MessageChannel channel) {
        String s = "Did you ever hear the tragedy of Darth Plagueis the Wise?\n"
                + "*No*\n"
    ...
    public void joke(MessageChannel channel) {
        Scanner scan = new Scanner(getClass().getClassLoader().getResourceAsStream("jokes.txt"));
    ...
    public void compliment(MessageChannel channel, List<Member> mentions) {
        String s = getCompliment();
    ...
    private void complimentEveryone(MessageChannel channel) {
        String s = getCompliment();
        channel.sendMessage("@everyone " + s).queue();
    }

    private String getCompliment() {
        Scanner scan = new Scanner(getClass().getClassLoader().getResourceAsStream("compliments.txt"));
        Random rand = new Random();
